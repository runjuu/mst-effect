...
import { types, dollEffect, action, destroy, NOOP, ValidEffectActions } from '../src'

describe('doll-effect', () => {
  describe(`action`, () => {
    it(`should execute the action immediately`, () => {
      const spy = jest.fn()
      const Model = types.model().actions((self) => ({
        log: dollEffect<string>(self, (payload$) =>
    ...
    it(`should accept 'NOOP' as valid action`, () => {
      const spy = jest.fn()
      const Model = types.model().actions((self) => ({
        run: dollEffect<ValidEffectActions>(self, (payload$) => payload$),
      }))
    ...
    it(`should accept an array of actions`, () => {
      const spy = jest.fn()
      const Model = types.model().actions((self) => ({
        log: dollEffect<number>(self, (payload$) =>
          payload$.pipe(
    ...
    it(`should able to modify model inside action`, () => {
      ...
      const Model = types.model({ value: types.string }).actions((self) => ({
        setValue: dollEffect<string>(self, (payload$) => {
    ...
    it(`should warning if action invalid`, () => {
      ...
      const Model = types.model().actions((self) => ({
        emit: dollEffect<any>(self, (payload$) => payload$),
      }))
    ...
    it(`should warning if 'types' is not imported from 'mst-effect'`, () => {
      ...
      const Model = mstTypes.model().actions((self) => ({
        emit: dollEffect(self, (payload$) => payload$.pipe(map(() => NOOP))),
      }))
  ...
  describe(`dispatch function return value`, () => {
    ...
    it(`should resolve with undefined if did not trigger 'resolve' but the model was destroyed`, async () => {
      const Model = types.model().actions((self) => ({
        sendMsg: dollEffect<string>(self, (payload$) => payload$.pipe(map(() => NOOP))),
      }))
    ...
    it(`should able to use RxJS to process the resolve value`, async () => {
      const Model = types.model().actions((self) => ({
        dispatch: dollEffect<void, number>(self, (payload$, resolve) =>
          payload$.pipe(
    ...
    it(`should able to use 'endWith' to provide default value`, async () => {
      const Model = types.model().actions((self) => ({
        sendMsg: dollEffect<void, string>(self, (payload$, resolve) =>
          payload$.pipe(
            map(() => NOOP),
  ...
  it(`should keep alive after error occur`, () => {
    ...
    const Model = types.model().actions((self) => ({
      throwErr: dollEffect<boolean>(self, (payload$) =>
        payload$.pipe(
          map((throwErr) => {
            if (throwErr) {
              throw new Error('')
